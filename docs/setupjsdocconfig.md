# Configuration File (.eslintrc.js) for JSDoc Plugin

See the article on [Set Up for JSDoc Plugin](setupjsdoc.md) for instructions on how to use this.

``` javascript
/* eslint-disable prettier/prettier */

// This file is the default configuration file for TypeScript Analyzer (ESLint)
// The default location in C# is Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "TypeScriptAnalyzerConfig")
...
const config = {
    "root": true,
    //"ignorePatterns": ["node.d.ts"],  // This will work with a local .eslintrc.js: paths are relative to path of .eslintrc.js
    ...
    // Instead all rules are defined in appropriate overrides sections by file extension, even though we're not actually overriding anything
    "overrides": [
        {
