...
echo Running build.cmd to create folder Shared\Node
if exist "%~dp0..\src\Shared\Node\log.txt" echo Nothing to do - Shared\Node\log.txt already exists & goto:done

echo Deleting and recreating folder Shared\Node... 
if exist "%~dp0..\src\Shared\Node" rmdir /s /q "%~dp0..\src\Shared\Node"
mkdir "%~dp0..\src\Shared\Node"

echo Copying and unzipping core files (node.exe, server.js)...
copy /y "%~dp0..\src\TypeScriptAnalyzerEslintLinter\Node\*.*" "%~dp0..\src\Shared\Node"
pushd "%~dp0..\src\Shared\Node"
7z.exe x -y node.7z
del /q 7z.dll
del /q 7z.exe
del /q node.7z
